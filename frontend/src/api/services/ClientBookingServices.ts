...
export class ClientBookingService {
  ...
  async getAvailableDates(serviceID: number, bookingID: any) {
    return FetchWrapper.post(`client/booking/dates`, { serviceID, bookingID });
  }


  async getTimeslots(serviceID: number, serviceDurationID: number, bookingDate: Dayjs | string | null, bookingID: any) {
    return FetchWrapper.post(`client/booking/timeslots`, { serviceID, serviceDurationID, bookingDate, bookingID });
  }

  async getClientCarer(serviceID: number, duration: number, bookingDateTime: string, bookingID: any) {
  ...
  async updateBookingStatus(BookingID: string | number, BookingStatus: number, CarerID: number | string | null, userID: any, reason?: string, cancelAll?: boolean, cancelledByAdmin?: boolean | null) {
