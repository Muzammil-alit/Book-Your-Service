...
import { getClientList } from './action'
...
export type ClientTypeWithAction = {
  ClientName: string;
  EmailID: string;
  PhoneNo: string;
  CreatedOn?: string;
  ...
  = () => {
    ...
    const fetchBookings = async () => {
      ...
      try {
        ...
        const apiBookings = await getClientList();
        setTableData(apiBookings);
    ...
    const [columnVisibility, setColumnVisibility] = useState(() => {
      ...
      if (typeof window !== 'undefined') {
        ...
        const savedVisibility = localStorage.getItem('settingsClientTableColumnVisibility')
    ...
    useEffect(() => {
      localStorage.setItem('settingsClientTableColumnVisibility', JSON.stringify(columnVisibility))
    }, [columnVisibility])
    ...
    const handleResetColumnVisibility = () => {
      ...
      localStorage.setItem('settingsClientTableColumnVisibility', JSON.stringify(defaultVisibility))
    }
    ...
    const columns = useMemo<ColumnDef<ClientTypeWithAction, any>[]>(
      () => [
        ...
        columnHelper.accessor('ClientName', {
          ...
          cell: ({ row }) => (
            <div className='flex items-center gap-3'>
              {getAvatar({ avatar: "", fullName: row.original.ClientName })}
        ...
        columnHelper.accessor('PhoneNo', {
          header: 'Phone',
          cell: ({ row }) => <Typography className='font-medium'>{row.original.PhoneNo}</Typography>
        }),
    ...
    return (
      <>
        ...
        <EditUserInfo
          ...
          userID={selectedUserData?.ClientID ?? 0}
          setData={setData}
