...
import { Chip, Divider, Tooltip, Button, FormGroup, FormControlLabel, Checkbox, } from '@mui/material'
...
const RosterListTable = ({ filters, setFilters }) => {
  ...
  const [selectedCarer, setSelectedCarer] = useState(carerIdFromURL ? carerIdFromURL : '0'); // Default to "All" carer
  ...
  const [columnVisibility, setColumnVisibility] = useState(() => {
    ...
    if (typeof window !== 'undefined') {
      ...
      const savedVisibility = localStorage.getItem('RosterAdminColumnVisibility')
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      localStorage.setItem('RosterAdminColumnVisibility', JSON.stringify(columnVisibility));
    }
  }, [columnVisibility]);
  ...
  const handleResetColumnVisibility = () => {
    ...
    if (typeof window !== 'undefined') {
      localStorage.setItem('RosterAdminColumnVisibility', JSON.stringify(defaultVisibility));
    }
  }
  ...
  const fetchBookings = async () => {
    try {
      ...
      const filtersToPost = {
        ...
        carerID: selectedCarer !== '0' ? selectedCarer : null,
      }

      const apiBookings = await getAdminRosterApiCall(filtersToPost);

      const bookingsArray: BookingRequestType[] = Array.isArray(apiBookings) ? apiBookings : [];
  ...
  const columns = useMemo<ColumnDef<BookingRequestTypeWithAction, any>[]>(
    () => [
          ...
          return (
            <Typography className='font-medium'>
              {row.original.ActualStartDateTime ?
                formatTimeTo(row.original.ActualStartDateTime?.split('T')[1]) :
                ''
              }
            </Typography>
          );
          ...
          return (
            <Typography className='font-medium'>
              {row.original.ActualEndDateTime ?
                formatTimeTo(row.original.ActualEndDateTime?.split('T')[1]) :
                ''
              }
            </Typography>
          );
  ...
  const table = useReactTable<BookingRequestTypeWithAction>({
    data: data as BookingRequestTypeWithAction[],
    columns,
    filterFns: {
      fuzzy: fuzzyFilter
    },
  ...
  return (
    <>
      <Card>
        ...
        <div className="flex flex-wrap justify-between items-end gap-4 px-4 my-3">
          {/* Filters Section */}
