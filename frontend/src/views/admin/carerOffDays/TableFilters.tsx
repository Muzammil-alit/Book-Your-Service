...
const TableFilters: React.FC<TableFiltersProps> = ({ setData, tableData, loading }) => {
    const initialRange = "All";
    ...
    const applyFilters = () => {
        ...
        if (state.dateFrom && state.dateTo) {
            filtered = filtered.filter(record => {
                const recordDate = dayjs(record.OffDayDate);
                return recordDate.isAfter(state.dateFrom) && recordDate.isBefore(state.dateTo);
            });
        } else if (state.dateFrom) {
            filtered = filtered.filter(record =>
                dayjs(record.OffDayDate).isAfter(state.dateFrom)
            );
        } else if (state.dateTo) {
            filtered = filtered.filter(record =>
                dayjs(record.OffDayDate).isBefore(state.dateTo)
            );
        }
    ...
    const handleFromDateChange = (newDate: Dayjs | null) => {
        const date = newDate && newDate.isValid() ? newDate.startOf('day') : null;
        setState(prevState => ({
            ...prevState,
            dateFrom: date,
    ...
    const handleToDateChange = (newDate: Dayjs | null) => {
        const date = newDate && newDate.isValid() ? newDate.endOf('day') : null;
        setState(prevState => ({
            ...prevState,
            dateTo: date,
            selectedRange: "Custom",
