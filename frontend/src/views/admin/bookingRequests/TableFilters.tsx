...
interface TableFiltersProps {
    onFilterChange: (dateFrom: string, dateTo: string, userID: string, bookingType: number | null) => void;
}
...
const TableFilters: React.FC<TableFiltersProps> = ({ onFilterChange }) => {
    ...
    const handleUserChange = (event: SelectChangeEvent) => {
        ...
        onFilterChange(
            ...
            value,
            state.bookingType == 'all' || state.bookingType == null ? null : state.bookingType == 'recurring' ? 2 : 1
        );
    };

    const handleBookingTypeChange = (event: SelectChangeEvent) => {
        const value = event.target.value
        setState(prevState => ({
            ...prevState,
            bookingType: value
        ...
        onFilterChange(
            ...
            state.dateTo ? state.dateTo.format() : '',
            state.selectedUser,
            value == 'all' ? null : value == 'recurring' ? 2 : 1
        );
    };
    ...
    const handleRangeChange = (event: SelectChangeEvent<string>) => {
        ...
        if (range === "Custom") {
            ...
            onFilterChange('', '', state.selectedUser, state.bookingType == 'all' || state.bookingType == null ? null : state.bookingType == 'recurring' ? 2 : 1);
        } else if (dateRange) {
            setState(prevState => ({
                ...prevState,
                selectedRange: range,
    ...
    return (
        <div className="flex flex-wrap gap-4 py-2 ">
            <>
                {/* User dropdown */}
                <FormControl fullWidth sx={{ width: 170, backgroundColor: 'white' }}>
                    <InputLabel id="user-select-label">Status</InputLabel>
                    <Select
                ...
                <FormControl fullWidth sx={{ width: 170, backgroundColor: 'white' }}>
                    ...
                    <Select
                        ...
                        value={state.bookingType === null ? 'all' : state.bookingType}
